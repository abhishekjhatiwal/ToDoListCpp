...
int del(int k){
    ...
    if (k>v.size() || k<1)
    {
        cout<<"ERROR...! task #"<<k<<" does't exist! No task is delete! "<<endl;
    }
    else{
        ofstream write;
        write.open("todo.txt");
...
int ls(){
    ...
    if (v.size() == 0)
    {
        cout<<"There no pending todos. "<<endl;
    }
    else
    {
...
int help(){
    ...
    cout<<"$ .\ttodoapp ls                    #Show remaining todo item"<<endl;
